    ...
    <section ref={containerRef} className="relative bg-background pt-32 pb-24 md:pt-48 md:pb-40">
      ...
      <div className="px-5 mb-24 md:mb-32 services-hero-title max-w-[96rem] mx-auto text-center md:text-left">
        ...
        <h1 className="text-4xl md:text-6xl font-light leading-[1.1] tracking-[-0.04em] text-white">
          Sistemas construidos <br className="hidden md:block" /> para <span className="font-semibold italic">escalar.</span>
        </h1>
      </div>
            ...
            return (
              <div key={service.title} className={`service-block flex flex-col md:flex-row gap-10 md:gap-16 items-center ${!isEven ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full md:w-1/2">
                  <div className="relative w-full aspect-[4/3] md:aspect-square max-h-[600px] rounded-[2rem] overflow-hidden border border-white/10 bg-white/[0.02] shadow-[0_0_50px_rgba(0,191,203,0.05)] group">
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] bg-calipsoGlow/10 rounded-full blur-[80px] pointer-events-none transition-opacity duration-700 group-hover:opacity-100 opacity-60" />
                  </div>
                </div>
                ...
                <div className="w-full md:w-1/2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {/* Capacidades */}
                    <div className="p-7 rounded-[1.5rem] border border-white/5 bg-white/[0.02] transition-colors hover:border-calipso-500/30 hover:bg-white/[0.04]">
                      ...
                      <ul className="flex flex-col gap-4">
                        {service.bullets.map(b => (
                          <li key={b} className="flex items-start gap-3 text-white/90 text-sm font-medium">
                            <span className="w-1.5 h-1.5 shrink-0 rounded-full bg-calipso-400 shadow-[0_0_10px_#00BFCB] mt-1.5" />
                            {b}
                          </li>
                        ))}
                      </ul>
